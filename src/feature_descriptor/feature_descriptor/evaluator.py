...
class Evaluator:
    ...
    def __init__(self, backend, motion_model):
        ...
        # Pipeline
        self._context = EvalContext(backend, motion_model, self._device, self._batch_size)
    ...
    def on_input(self, image, depth):
        ...
        if len(self._batch_images) == self._batch_size:
            ...
            Evaluator.on_batch(self._context,
                np.array(self._batch_extrinsics), 
                np.array(self._batch_images), 
                np.array(self._batch_depths)
            )
    ...
    def on_batch(context, extrinsics, image, depth):
        # Enable training mode
        context.backend.get_model().eval()
        ...
        (last_kps, last_dess) = Evaluator.get_sparse_from_batch(kps, dess, 0)
        last_coords = Evaluator.extract_world_coords(context.motion_model, extrinsics[0], torch_depth[0], last_kps)
        ...
        for i in range(1, context.batch_size):
            ...
            (batch_kps, batch_dess) = Evaluator.get_sparse_from_batch(kps, dess, i)
            ...
            batch_loss += Evaluator.loss(distance_matrix, match_matrix)

            # Latch the current values, so that they are available for the next iteration
            last_kps = batch_kps
            last_dess = batch_dess
            last_coords = Evaluator.extract_world_coords(context.motion_model, extrinsics[i], torch_depth[i], batch_kps)

        print(f'Loss: {batch_loss.item() / context.batch_size}')

        if len(match_matrix) > 0:
            epsilon_d = 8

            tops = torch.topk(match_matrix, k=2, dim=0)
            relevant_matches = tops.values[0] > 0.99
            predicted_matches = tops.indices[0, relevant_matches]
            evaluated_distances = distance_matrix[predicted_matches, relevant_matches]
            correct_matches = evaluated_distances < epsilon_d
            possible_matches = distance_matrix < epsilon_d
            
            num_predicted_matches = torch.count_nonzero(predicted_matches).item()
            num_correct_matches = torch.count_nonzero(correct_matches).item()
            num_possible_matches = torch.count_nonzero(possible_matches).item()
                        
            if num_predicted_matches > 0 and num_possible_matches > 0:
                print("Precision: " + str(num_correct_matches / num_predicted_matches) + "; Recall: " + str(num_correct_matches / num_possible_matches))
                print("Possible matches: " + str(num_possible_matches) + "; Predicted matches: " + str(num_predicted_matches))
            else:
                print("No match found")
                
            # (x1_coords, y1_coords) = (last_kps[:, 1], last_kps[:, 0])
            # (x2_coords, y2_coords) = (expected_kps[:, 1], expected_kps[:, 0])
                
            # plt.figure(figsize=(8, 6))
            # plt.imshow(image[self._batch_size-1])
            # plt.scatter(x1_coords.cpu(), y1_coords.cpu(), color='blue', s=1)
            # plt.scatter(x2_coords.cpu(), y2_coords.cpu(), color='red', s=1)
