...
from astronet_utils import ProjectionUtils, PointsUtils, MotionUtils
    ...
    def loop(self):
        ...
        while count < self._size:
            ...
            if MotionUtils.is_valid(data):
                ...
                distance_matrix = (reproj_points_2D[:, None, :] - next_points_2D.kps[None, :, :]).squeeze().norm(dim=2)
                filter = crit.apply(distance_matrix).to(dtype=torch.bool)
        ...
        plt.figure()
        plt.hist(torch.hstack(dist_stats).cpu().numpy(), bins="auto")
        plt.show()