...
class ExtrinsicsUtils(Batchable, Torchable, Filterable):
    # Constructor helpers
    ...
    def apply(extrinsics, point):
        ...
        return (quad[:, 0:3]/quad[:, 3, None]).reshape(-1, 3) # Always normalize by the homogenesous coordinate

    def revert(extrinsics, point):
        quad = torch.einsum("ij,...j->...i", extrinsics.M_inv, torch.hstack((point, torch.ones(point.shape[0], 1))))
        return (quad[:, 0:3]/quad[:, 3, None]).reshape(-1, 3) # Always normalize by the homogeneous coordinate
    ...
    def detach(extrinsics):
        M = extrinsics.M.detach()
        M_inv = extrinsics.M_inv.detach()
        return ProjectionData.ExtrinsicsData(M, M_inv)
    
    # Compliance with Filterable abstraction
    def filter(extrinsics, filter):
        M = extrinsics.M[filter]
        M_inv = extrinsics.M_inv[filter]
        return ProjectionData.ExtrinsicsData(M, M_inv)
