...
class KLLoss(LossFunction):
    ...
    def loss(self, true_dists, true_matches, pred_dists, pred_matches):       
        true_bins = torch.maximum(torch.tensor(0), true_matches)
        ...
        sigma = 2
        noise_sigmas = 2 # Number of sigmas above which the signal is considered an error
        num_entries = truth.shape[1]
        noise_likelihood = -(noise_sigmas)**2/2 * torch.ones((1, num_entries))
        expected = torch.log_softmax(torch.vstack((-true_dists**2/(2*sigma*sigma), noise_likelihood)), dim=0)

        kl_div = torch.sum(torch.exp(expected) * (expected - torch.log(torch.maximum(torch.tensor(1e-12), pred))), dim=0)
