...
class Trainer:
    def __init__(self, train_frontend, validate_frontend, train_size, validate_size, output_params):
        ...
        if output_params["type"] != "PTH":
            raise NotImplementedError("Supported output only includes Pytorch PTH model")
        
        self._pth_path = output_params["path"]
        
        # Hyperparameters
        self._batch_size = 16
        self._max_epochs = 500
        self._epsilon = 8/1024
        lr = 0.001
        momentum = 0.9
        weight_decay=0.00
        ...
        self._features_matcher = Matcher(Cosine, PassThroughCriterion())
        ...
        # Debugging tools
        self._visualizer = Visualizer(1024, 256)
    ...
    def loop(self):
        for epoch in range(self._max_epochs):
            ...
            avg_accuracy = torch.tensor(list(map(lambda x: x.accuracy(), val_stats))).mean()
            avg_precision = torch.tensor(list(map(lambda x: x.precision(), val_stats))).mean()
            avg_recall = torch.tensor(list(map(lambda x: x.recall(), val_stats))).mean()
            avg_f1 = torch.tensor(list(map(lambda x: x.f1(), val_stats))).mean()

            # Log statistics
            print(f"(Pseudo-)epoch {epoch} finished with train loss {avg_train_loss:.4}, val loss {avg_val_loss:.4}, accuracy {avg_accuracy:.1%}, precision {avg_precision:.1%}, recall {avg_recall:.1%}, f1 {avg_f1:.1%}")
            wandb.log({"Accuracy": avg_accuracy, "Precision": avg_precision*100, "Recall": avg_recall*100, "F1-Score": avg_f1*100, "Train loss": avg_train_loss, "Validation Loss": avg_val_loss})
    ...
    def loss(self, true_dists, true_matches, pred_dists, pred_matches):
        ...
        pred_dists = torch.nn.functional.relu(pred_dists)
        ...
        match_prob = (2*1024*self._epsilon)/true_matches.size(0) # Average number of features
        margin = 0.2

        tp_loss = true_matches * torch.max(torch.tensor(0), (1 - pred_dists)) / torch.count_nonzero(true_matches)
        tn_loss = (1-true_matches) * torch.max(torch.tensor(0), pred_dists - margin) / torch.count_nonzero(1-true_matches)
        
        hinge_loss = (1-match_prob)*tp_loss + match_prob*tn_loss
        
        return hinge_loss.sum()#torch.nn.functional.binary_cross_entropy_with_logits(pred_dists, true_matches)
    ...
    def forward_sparse(self, input_coords, input_features):
        ...
        output_features = torch.nn.functional.normalize(output.features, dim=1)
    ...
    def get_sample(self, batch, idx):
        ...
        return (c1[filter1, 1:3] / 1024, c2[filter2, 1:3] / 1024, f1[filter1], f2[filter2])